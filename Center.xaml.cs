...
namespace IERG3080_____Pokemon_Go
{
    ...
    public partial class Center : Page
    {
        ...
        private TimeSpan interval = new TimeSpan(0, 1, 13);
        bool flag = true;
        ...
        public Center()
        {
            ...
            center_map.Source = (GetPicture.Source("center")).Source;
            enter.Source = (GetPicture.Source("Enter")).Source;

            string link = GetPicture.Source_mp3("center");
        ...
        private void Tutor_Click(object sender, RoutedEventArgs e)
        {
            ...
            MessageBox.Show("Chat with the staff in the red rectangular, then you can manage your pokemon collection !");
        }
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            if (check_intersect(healer) || check_intersect(exit))
            {
                ...
                if (goEnter == true && check_intersect(healer))
                {
                    flag = false;
                    goEnter = false;

                    map_info info = map_info.get_map();
                    info.set_map(Canvas.GetTop(player), Canvas.GetLeft(player), 2);
            ...
            else if (goDown && Canvas.GetTop(player) + (player.Height * 2) < 515 && bot)
            {
                Canvas.SetTop(player, Canvas.GetTop(player) + speed);
        ...
        {
            Rect player_hitbox = BoundsRelativeTo(player, Center_map);
            Rect block = BoundsRelativeTo(obj, Center_map);
